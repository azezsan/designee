...
<section use:inview={options} on:enter={() => (lazyLoad = true)} class="bg-base-300 py-20 sm:py-32 relative" bind:clientWidth={width}>
	<div class="mx-auto max-w-7xl px-4 md:px-6 lg:px-8 grid gap-24">
		...
		<section class="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3  ">
			{#each chunkArray(reviews, handleResize(width)) as chunk, i}
				<ul class="animate-marquee space-y-8 py-4" style="--marquee-duration:{getDuration(i)};">
					{#each Array(2) as _}
			...
			<div class="pointer-events-none absolute inset-x-0 top-0 h-32 bg-base-300-gradient" />
			<div class="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-base-300" />
		</section>
	</div>
</section>